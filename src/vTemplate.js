(function(Global) {
  var consts = {
    "stplReg": /\<(\=)([^\<\=\>]+?)\>|\<(\@)([^\<\@\=]+?)\(\s*(?:\w+(?:\.\w+)*)(?:\s*\,\s*(?:\w+(?:\.\w+)*)){0,}\s*\)/gm,
    "arrPush": Array.prototype.push,
    "keywords": ["alert", "break", "case", "catch", "continue", "debugger",
      ...
      "console", "arguments", "setTimeout", "setInterval", "clearTimeout", "clearInterval"]
  };
  var prefix = {
    "str": "out+=\'",
    "arrKw": "out+=",
  ...
  function __compile__(stpl, config) {
    ...
    while(step = stplReg.exec(stpl)) {
      compiled += prefix.str + stpl.slice(oldIndex, stplReg.lastIndex - step[0].length) + postfix.str;
      ...
      if(step[1] == "\=") {
        ...
        compiled += (inArray(ignored, kw) ? prefix.arrKw : prefix.attr) + step[2] + postfix.semicolon;
      }else if(step[3] == "\@") {
        ...
        var mapStpl = map[1];
        var match = step[0];
        // if(step[4]=="if")debugger
        // find out the helpers arguments
        var args = match.slice(match.indexOf("(")+1, match.indexOf(")")).replace(/(\w+(\.\w+)*)/gm,"\"$1\"");
    ...
    var tail = prefix.str+stpl.slice(oldIndex)+postfix.str;
  ...
  var helpers = {
    ...
    "if": function(condition, stpl, ignored) {
      var _ignored = [];
      consts.arrPush.apply(_ignored, ignored);
      consts.arrPush.apply(_ignored, consts.keywords);
      var word = /(\w+)(\.\w+)*/gm;
      condition = condition.replace(word, function(m0, m1) {
        console.log(m0)
        if(!inArray(_ignored, m1)) return helpers.prefix + m0
      });
      var compiled = __compile__(stpl, {
        "ignored": ignored
      });
